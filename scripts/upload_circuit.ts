...
async function main() {
  ...
  const idl = JSON.parse(fs.readFileSync("target/idl/encrypted_identity.json", "utf-8"));
  const program = new anchor.Program(idl, provider) as anchor.Program<any>;

  console.log("Uploading add_together circuit (encrypted_identity) to cluster 456...");
